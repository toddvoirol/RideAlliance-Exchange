...
import { TripCancellationReportService } from './trip-cancellation-report.service';
...
import {
  CancellationRange,
  ShowTicketOption,
  TripCancellationReport,
} from './trip-cancellation-report.model';
...
@Component({
  selector: 'app-trip-cancellation-report',
  templateUrl: './trip-cancellation-report.component.html',
  styleUrls: ['./trip-cancellation-report.component.scss'],
})
export class TripCancellationReportComponent implements OnInit, OnDestroy {
  ...
  // Filter parameters
  public cancel: CancellationRange = {
    fromDate: '',
    toDate: '',
    providerId: 0,
  ...
  // Data
  public ticketList: TripCancellationReport[] = [];
  ...
  public pageTitle: string = 'Cancelled Trips Report';
  ...
  /**
   * Filter trip cancellation reports based on selected criteria
   */
  public filter(): void {
    this.isFilter = true;
    ...
    if (this.loggerRole === 'ROLE_ADMIN') {
      this.cancel.providerId = this.providerPartnersId as number;
    } else {
      this.cancel.providerId = Number(this._localStorage.get('providerId'));
    }
    ...
    this.cancel.fromDate = dateFrom.split('+')[0];
    this.cancel.toDate = dateTo.split('+')[0];

    // Set ticket type filters (My Tickets/Partner Tickets)
    this.cancel.myTicket = false;
    this.cancel.partnerProviderTicket = false;

    if (this.arrayShowTicket.length === 0) {
      // If no checkboxes selected, show all tickets
      this.cancel.partnerProviderTicket = true;
      this.cancel.myTicket = true;
    } else {
    ...
    // Fetch the cancelled ticket list
    this.loading = true;
    this._tripCancellationReportService
      .getCancelledTicketList(this.cancel)
      .pipe(takeUntil(this.destroy$))
      .subscribe({
        next: response => {
          this.loading = false;
        ...
        error: error => {
          this.loading = false;
          console.error('Error filtering trip cancellation reports:', error);
          this._notificationService.error(
            this.translateService.instant('COMMON.ERROR'),
   ...
   */
  private formatTripDetails(ticket: TripCancellationReport): void {
  ...
  public exportToExcel(): void {
    ...
    this.saveAsExcelFile(excelBuffer, 'cancelled_trips_report');
  }
  ...
  public exportToCSV(): void {
    ...
    link.setAttribute('download', 'cancelled_trips_report.csv');
